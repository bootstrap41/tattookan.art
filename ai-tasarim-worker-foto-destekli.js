...
export default {
  async fetch(request, env) {
    ...
    const photoDataUrl = body.image ? String(body.image) : null;
    ...
    // --- 4) OpenAI'a görsel isteği gönder ---
    // Fotoğraf yoksa: sıfırdan tasarım üretimi (generations).
    // Fotoğraf varsa: müşterinin fotoğrafını düzenleyip üzerine tasarım işleme (edits).
    let openaiData;

    try {
      if (photoDataUrl) {
        const commaIndex = photoDataUrl.indexOf(",");
        const base64Part = commaIndex >= 0 ? photoDataUrl.slice(commaIndex + 1) : photoDataUrl;
        ...
        const editPrompt =
          "Add a realistic tattoo design onto the visible skin in this photo, in the exact spot that " +
          "makes sense anatomically. Style: black and white line art / fine line tattoo ink, looking like " +
          "real tattoo ink on skin (not a sticker or drawing overlay). Keep the rest of the photo, the " +
          "person, lighting, and background completely unchanged. Design to add: " +
          userPrompt;

        const formData = new FormData();
        formData.append("model", "gpt-image-1");
        formData.append("image", new Blob([bytes], { type: "image/png" }), "photo.png");
        formData.append("prompt", editPrompt.slice(0, 900));
        ...
        if (!openaiRes.ok) {
          ...
          console.error("OpenAI (edit) hata:", errText);
          return jsonResponse({ error: "Fotoğraf işlenemedi, lütfen tekrar dene ya da fotoğrafsız devam et." }, 502);
        }
      ...
      await fetch("https://api.resend.com/emails", {
        ...
        body: JSON.stringify({
          ...
          html:
            ...
            `<p><strong>Tür:</strong> ${photoDataUrl ? "Müşteri fotoğrafı üzerine işlendi" : "Sıfırdan tasarım"}</p>` +
            `<p><strong>Kullanılan kod:</strong> ${escapeHtml(code)} (kalan hak: ${remainingUses - 1})</p>` +
            `<p><strong>İstek:</strong> ${escapeHtml(userPrompt)}</p>` +
            `<p><strong>Müşteri telefon numarası:</strong> ${escapeHtml(contact)}</p>`,
          attachments: [
            {
